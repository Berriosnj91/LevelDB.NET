...
internal static class SnappyDecompressor
{
    ...
    private static int UpdateSum(IReadOnlyList<byte> data, int offset, int currentShift, int sum)
    {
    ...
    public static int Decompress(byte[] input, int inputOffset, int inputSize, byte[] output, int outputOffset,
        int outputLimit)
    {
    ...
    private static void CopyLiteral(byte[] input, int ipIndex, byte[] output, int opIndex, int length)
    {
        ...
        if (readableBytes < length || spaceLeft < length) throw new InvalidOperationException("Corrupt literal length");
