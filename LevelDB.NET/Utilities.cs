...
{
    private static readonly uint[] T80 =
    {
    ...
    private static readonly uint[] T81 =
    {
    ...
    private static readonly uint[] T82 =
    {
    ...
    private static readonly uint[] T83 =
    {
    ...
    private static readonly uint[] T84 =
    {
    ...
    private static readonly uint[] T85 =
    {
    ...
    private static readonly uint[] T86 =
    {
    ...
    private static readonly uint[] T87 =
    {
    ...
    /// <remarks>
    ///     The name comes from the original Snappy C++ source. I don't think there is a good way to look at
    ///     things in an aligned manner in the .NET Framework.
    /// </remarks>
    ...
    public static uint NumberOfTrailingZeros(ulong c)
    {
        ...
        if (matchingBits == 32) matchingBits += NumberOfTrailingZeros((uint)(c >> 32));
        return matchingBits;
    }
    ...
    public static uint CalculateCrc(byte[] b, int off, int len)
    {
        ...
        while (len > 7)
        {
            ...
            var c1 = b[off++] ^ (localCrc >>= 8);
            var c2 = b[off++] ^ (localCrc >>= 8);
            var c3 = b[off++] ^ (localCrc >>= 8);
            localCrc = T87[c0 & 0xff] ^ T86[c1 & 0xff]
                                       ^ T85[c2 & 0xff] ^ T84[c3 & 0xff];

            localCrc ^= T83[b[off++] & 0xff] ^ T82[b[off++] & 0xff]
                                              ^ T81[b[off++] & 0xff] ^ T80[b[off++] & 0xff];
        ...
        while (len > 0)
        {
            localCrc = (localCrc >> 8) ^ T80[(localCrc ^ b[off++]) & 0xff];
            len--;
        }
